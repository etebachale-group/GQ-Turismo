...
if (!isset($_GET['id']) || !filter_var($_GET['id'], FILTER_VALIDATE_INT)) {
    echo "<div class='container py-5'><div class='alert alert-danger text-center'>ID de agencia no válido o no proporcionado.</div></div>";
...
if ($conn) {
    ...
    if ($result->num_rows > 0) {
        ...
        $stmt_services->bind_param("i", $id_agencia);
        ...
        $stmt_menus->bind_param("i", $id_agencia);
        $stmt_menus->execute();
...
<div class="container py-5">
        ...
        <div class="row g-5">
            ...
            <div class="col-lg-4">
                <div class="card shadow-sm">
                    <div class="card-body">
                        <h5 class="card-title">Contactar Agencia</h5>
                        <p class="card-text">¿Interesado en los servicios de esta agencia? Contáctalos directamente.</p>
                        <a href="mailto:<?= htmlspecialchars($agencia['contacto_email']) ?>" class="btn btn-primary w-100 mb-2">Enviar Email</a>
                        <?php if ($agencia['contacto_telefono']): ?>
                            <a href="tel:<?= htmlspecialchars($agencia['contacto_telefono']) ?>" class="btn btn-outline-secondary w-100 mb-2">Llamar</a>
                        <?php endif; ?>
                        <?php if (isset($_SESSION['user_id']) && $_SESSION['user_type'] === 'turista'): ?>
                            <button type="button" class="btn btn-success w-100" data-bs-toggle="modal" data-bs-target="#sendMessageModal" data-receiver-id="<?= htmlspecialchars($agencia['id']) ?>" data-receiver-type="agencia" data-receiver-name="<?= htmlspecialchars($agencia['nombre_agencia']) ?>">Enviar Mensaje</button>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
    ...
    <?php else: ?>
        <div class="text-center py-5">
            <h2 class="fw-bold">Agencia no encontrada</h2>
            <p class="text-muted">La agencia que buscas no existe o no está disponible en este momento.</p>
            <a href="agencias.php" class="btn btn-primary mt-3">Volver a la lista de Agencias</a>
        </div>
    <?php endif; ?>
</div>
