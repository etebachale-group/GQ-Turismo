...
$imagenes = [];
$servicios = [];
$turista_itinerarios = []; // Para almacenar los itinerarios del turista
...
if ($conn) {
    ...
    if ($result->num_rows > 0) {
        ...
        // Si el usuario es un turista logueado, obtener sus itinerarios
        if (isset($_SESSION['user_id']) && $_SESSION['user_type'] === 'turista') {
            $stmt_itinerarios = $conn->prepare("SELECT id, nombre_itinerario FROM itinerarios WHERE id_usuario = ? ORDER BY fecha_creacion DESC");
            $stmt_itinerarios->bind_param("i", $_SESSION['user_id']);
            $stmt_itinerarios->execute();
            $result_itinerarios = $stmt_itinerarios->get_result();
            while ($row = $result_itinerarios->fetch_assoc()) {
                $turista_itinerarios[] = $row;
            }
